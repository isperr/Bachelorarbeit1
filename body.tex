...
As mentioned before, nowadays commercial search engines like Google and Yahoo! are extremely popular. For the user, they are easy to handle because they have a user-friendly interface \autocite{pan2007google}.

There has been done a lot of research on search engines, that conducted a study like \textcite{liu2015influence, buscher2010good} with an engine, that was adapted from a conventional commercial web search engine. 


\subsubsection{Comparison and Eye Tracking results}
As \textcite{pan2007google, wang2016beyond} are explaining in their paper, rank position of the results and the corresponding abstracts is one of the most noticeable and possibly most used comparison criteria when analyzing search engines. 
...
Another criterion that is used to analyze and compare search engines is the "ten blue links"-principle, also called organic results, combined with attention bias \autocite{buscher2010good, liu2015influence}. There are a few different sections on a modern SERP (see Figure \ref{figure:ModernSerp}). Some of these segments contain pictures, which essentially draw more attention to them then solely plain text. 
